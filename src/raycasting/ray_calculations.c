...
/*   Updated: 2025/07/16 16:43:24 by itsiros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
1. calculates the relative horizontal position of the current screen column
(camera_x)
2. calculates the direction of the ray 
		vec->dir_x & vec->dir_y:  direction the player is facing
		vec->plane_x & vec->plane_y: a vector perpendicular to the player
		direction
									(represents 2D "camera plane" (like FOV)
	to avoid fish-eye effect (instead of calculating with players and wall
	position)
3. calculates how far the ray has to go along the ray direction to move across
a grid line delta_dist_x & delta_dist_y
note: essential for DDA step where ray moves through the map grid
note: 1e30 is used to avoid division by 0 when ray goes straight
...
and figures out which part of the screen it should be drawn on to create
3D effect. 
1. uses the distance to the wall to compute how high the wall appears
(close = tall, far = short)
